                            ...
                            <a target="_blank" href="">
                                <div class="flex px-4 py-2 flex-auto flex-wrap items-center">
                                    <div class="text-white text-center inline-flex w-10 h-10">
                                        <img class="rounded-lg" id="preview-icon" src="{{ $item->social->icons->first() ? RvMedia::getImageUrl($item->social->icons->first()->image) : RvMedia::getDefaultImage() }}" alt="{{ $item->name }}">
                                    </div>
                            ...
                            <div class="relative w-full mb-3">
                                ...
                                <select name="social_id" id="social" data-url="{{ RvMedia::url('/') }}" class="select-2 px-3 py-3 border placeholder-gray-400 text-gray-700 bg-white rounded text-sm shadow focus:outline-none focus:shadow-outline w-full ease-linear transition-all duration-150">
                                    @foreach($socials as $social)
                                        <option @if($social->id == $item->social->id) selected @endif value="{{ $social->id }}" data-source="{{ $social }}" data-icon="{{ $social->icons->first() ? RvMedia::getImageUrl($social->icons->first()->image) : RvMedia::getDefaultImage() }}">{{ $social->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
